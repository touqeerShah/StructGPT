...
def infer_split_regex(
    ...
    for i, page in enumerate(pages):
        ...
        feedback = (
            f"\nHere was the previous regex attempt:\n{previous_pattern}\n"
            "It did not successfully split the text. Please improve it."
            if previous_pattern
            else ""
        )

        prompt = f"""
                You are an expert in analyzing text patterns for structured data extraction.
                Your main focus on the field , check in each chunk all field should present 
                Task:
                - Use the class structure below to understand how a single logical record is structured.
                - Analyze the input sample text.
                - Infer a **Python regex pattern** that can split the full document into sections that represent one complete instance of the class.
                - The regex must anchor around strong boundaries like unique IDs, section headers, repeated patterns, etc.

                Requirements:
                - Regex must include cues to separate records that contain **fields from the class structure**.
                - Output **only** a valid regex pattern, wrapped in triple backticks.
                ...
                ### Output:
                A single Python regex for re.split().
                ### Comments
                {feedback}
                {reason} 
            """
        ...
        try:
            ...
            if match:
                ...
                is_valid, reason = validate_regex_pattern_with_llm(
                    previous_pattern, page, class_structure, llm
                )
                if is_valid:
